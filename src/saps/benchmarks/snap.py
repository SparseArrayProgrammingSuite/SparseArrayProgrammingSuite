...
class SNAPDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        if group not in _GROUP_CONCEPTS:
            raise ValueError(f"Unknown SNAP group: {group!r}")
        self._name = name
        ...
        self._pretty_name = pretty_name or self.source_name
    ...
    def concepts(self) -> str:
        concept_id, description = _GROUP_CONCEPTS[self.group]
        return (
            "<ccs2012><concept>"
...
# One explicit entry per pasted group occurrence. Repeated sources have unique
# benchmark names but share source_name and the same raw download directory.
# Counts describe the published source, not the remapped prepared adjacency.
# fmt: off
# Columns: name, group, types, nodes, edges, description.
_GRAPHS = [
    # Social networks
    ...
    SNAPDataset("wiki-Vote", "Social networks", ["Directed"], 7115, 103689, "Wikipedia who-votes-on-whom network"),
    SNAPDataset("wiki-RfA", "Social networks", ["Directed", "Signed"], 10835, 159388, "Wikipedia Requests for Adminship (with text)"),
    ...
    SNAPDataset("soc-RedditHyperlinks", "Social networks", ["Directed", "Signed", "Temporal", "Attributed"], 55863, 858490, "Hyperlinks between subreddits on Reddit"),
    SNAPDataset("soc-sign-bitcoin-otc", "Social networks", ["Weighted", "Signed", "Directed", "Temporal"], 5881, 35592, "Bitcoin OTC web of trust network"),
    SNAPDataset("soc-sign-bitcoin-alpha", "Social networks", ["Weighted", "Signed", "Directed", "Temporal"], 3783, 24186, "Bitcoin Alpha web of trust network"),
    SNAPDataset("comm-f2f-Resistance", "Social networks", ["Weighted", "Directed", "Temporal"], 451, 3126993, "Dynamic face-to-face interaction network between group of people"),
    ...
    SNAPDataset("act-mooc", "Social networks", ["Bipartite", "Directed", "Attributed", "Temporal"], 7143, 411749, "Student actions on a MOOC platform, with student drop-out binary labels."),
    ...
    SNAPDataset("cisco-secure-workload", "Networks with ground-truth communities", ["Directed", "Communities", "Attributed"], "between 86 and 278,739", "between 155 and 2,158,346", "22 graphs of communications between computers, 2 have ground truth groupings", communities="10 and 23"),
    # Communication networks
    ...
    SNAPDataset("wiki-Talk", "Communication networks", ["Directed"], 2394385, 5021410, "Wikipedia talk (communication) network"),
    SNAPDataset("comm-f2f-Resistance__communication-networks", "Communication networks", ["Weighted", "Directed", "Temporal"], 451, 3126993, "Dynamic face-to-face interaction network between group of people", source_name="comm-f2f-Resistance"),
    # Citation networks
    ...
    # Signed networks
    SNAPDataset("soc-RedditHyperlinks__signed-networks", "Signed networks", ["Directed", "Signed", "Temporal", "Attributed"], 55863, 858490, "Hyperlinks between subreddits on Reddit", source_name="soc-RedditHyperlinks"),
    SNAPDataset("soc-sign-bitcoin-otc__signed-networks", "Signed networks", ["Weighted", "Signed", "Directed", "Temporal"], 5881, 35592, "Bitcoin OTC web of trust network", source_name="soc-sign-bitcoin-otc"),
    SNAPDataset("soc-sign-bitcoin-alpha__signed-networks", "Signed networks", ["Weighted", "Signed", "Directed", "Temporal"], 3783, 24186, "Bitcoin Alpha web of trust network", source_name="soc-sign-bitcoin-alpha"),
    SNAPDataset("soc-sign-epinions", "Signed networks", ["Directed"], 131828, 841372, "Epinions signed social network"),
    SNAPDataset("wiki-Elec", "Signed networks", ["Directed", "Bipartite"], "~7,000", "~100,000", "Wikipedia adminship election data"),
    SNAPDataset("wiki-RfA__signed-networks", "Signed networks", ["Directed", "Signed"], 10835, 159388, "Wikipedia Requests for Adminship (with text)", source_name="wiki-RfA"),
    ...
    SNAPDataset("wiki-Vote__wikipedia-networks-articles-and-metadata", "Wikipedia networks, articles, and metadata", ["Directed"], 7115, 103689, "Wikipedia who-votes-on-whom network", source_name="wiki-Vote"),
    SNAPDataset("wiki-Talk__wikipedia-networks-articles-and-metadata", "Wikipedia networks, articles, and metadata", ["Directed"], 2394385, 5021410, "Wikipedia talk (communication) network", source_name="wiki-Talk"),
    SNAPDataset("wiki-Elec__wikipedia-networks-articles-and-metadata", "Wikipedia networks, articles, and metadata", ["Bipartite"], "~7,000", "~100,000", "Wikipedia adminship election data", source_name="wiki-Elec"),
    SNAPDataset("wiki-RfA__wikipedia-networks-articles-and-metadata", "Wikipedia networks, articles, and metadata", ["Directed", "Signed"], 10835, 159388, "Wikipedia Requests for Adminship (with text)", source_name="wiki-RfA"),
    ...
    # Temporal networks
    SNAPDataset("soc-RedditHyperlinks__temporal-networks", "Temporal networks", ["Directed", "Signed", "Temporal", "Attributed"], 55863, 858490, "Hyperlinks between subreddits on Reddit", source_name="soc-RedditHyperlinks", static_edges=858490),
    ...
    SNAPDataset("soc-sign-bitcoin-otc__temporal-networks", "Temporal networks", ["Weighted", "Signed", "Directed", "Temporal"], 5881, 35592, "Bitcoin OTC web of trust network", source_name="soc-sign-bitcoin-otc", static_edges=None),
    SNAPDataset("soc-sign-bitcoin-alpha__temporal-networks", "Temporal networks", ["Weighted", "Signed", "Directed", "Temporal"], 3783, 24186, "Bitcoin Alpha web of trust network", source_name="soc-sign-bitcoin-alpha", static_edges=None),
    SNAPDataset("act-mooc__temporal-networks", "Temporal networks", ["Bipartite", "Directed", "Attributed", "Temporal"], 7143, 411749, "Student actions on a MOOC platform, with student drop-out binary labels.", source_name="act-mooc", static_edges=None),
    SNAPDataset("comm-f2f-Resistance__temporal-networks", "Temporal networks", ["Weighted", "Directed", "Temporal"], 451, 3126993, "Dynamic face-to-face interaction network between group of people", source_name="comm-f2f-Resistance", static_edges=None),
    # User actions
    SNAPDataset("act-mooc__user-actions", "User actions", ["Bipartite", "Directed", "Attributed", "Temporal"], 7143, 411749, "Student actions on a MOOC platform, with student drop-out binary labels.", source_name="act-mooc"),
    # Memetracker and Twitter
    ...
    # Online communities
    SNAPDataset("soc-RedditHyperlinks__online-communities", "Online communities", ["Subreddit hyperlinks"], None, None, "Hyperlinks between subreddits on Reddit", source_name="soc-RedditHyperlinks", items="858,490 links between 55,863 subreddits"),
    ...
    # Face-to-face communication networks
    SNAPDataset("comm-f2f-Resistance__face-to-face-communication-networks", "Face-to-face communication networks", ["Weighted", "Directed", "Temporal"], 451, 3126993, "Dynamic face-to-face interaction network between group of people", source_name="comm-f2f-Resistance"),
    # Graph classification datasets
    ...
    # Computer communication networks
    SNAPDataset("cisco-secure-workload__computer-communication-networks", "Computer communication networks", ["Directed", "Attributed"], None, None, "Communications among computers running different distributed applications in various enterprises", source_name="cisco-secure-workload", graphs=22),
    # Cryptocurrency transactions
...
class SNAPGraphGenerator(Generator[SNAPDataset]):
    ...
    def generate(self, dataset: SNAPDataset) -> DataInstance:
        inputs, meta = download_snap_dataset(
            dataset.source_name, data_dir=self.backend.cache_dir / "snap"
        )
