...
from sparseappbench.benchmarks.cp_als import (
    CP_ALS,
    CPFactorizeableGenerator,
)
import sparseappbench.benchmarks.cp_als as cp_als
...
def test_cp_als_reconstruction_error(xp):
    ...
    gen = CPFactorizeableGenerator()
    X_bin, rank, max_iter = gen.generate(gen.datasets[0])

    cp_als.xp = NumpyFramework()

    X = cp_als.xp.from_binsparse(X_bin)

    A_bin, B_bin, C_bin, lambda_bin = CP_ALS().benchmark(
        (X,), {"rank":rank, "max_iter":max_iter}
    )

    A = cp_als.xp.from_binsparse(A_bin)
    B = cp_als.xp.from_binsparse(B_bin)
    C = cp_als.xp.from_binsparse(C_bin)
    l = cp_als.xp.from_binsparse(lambda_bin)
