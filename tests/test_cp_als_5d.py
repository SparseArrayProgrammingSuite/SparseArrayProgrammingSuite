...
from sparseappbench.benchmarks.cp_als_5d import (
    benchmark_cp_als,
...
def test_cp_als_basic(xp):
    """Testing that CP-ALS in 5D runs without
    errors and produces correct output shapes"""
    X_bin, rank, max_iter = dg_cp_als_sparse_small()

    A_bin, B_bin, C_bin, D_bin, E_bin, lambda_bin = benchmark_cp_als(
...
def test_cp_als_reconstruction_error(xp):
    """Tests that CP-ALS in 5D produces low
    ...
    X_indices = (
        ...
        X_bin.data["indices_3"],
        X_bin.data["indices_4"],
    )
    ...
    D_vals = D_bin.data["values"].reshape(D_bin.data["shape"])
    E_vals = E_bin.data["values"].reshape(E_bin.data["shape"])
    lambda_vals = lambda_bin.data["values"]

    # Y[i,j,k] = sum_r lambda_r * A[i,r] * B[j,r] * C[k,r]
    i_idx, j_idx, k_idx, l_idx, m_idx = X_indices
